...
```
请注意，--allow-host并且--deny-host仅支持IP地址（不支持主机名）。上面的示例通过将--topic [topic-name]指定为资源模式选项来向主题添加ACL。同样，用户可以通过指定--cluster将acls添加到集群，并通过指定--group [group-name]将其添加到使用者组。您可以在特定类型的任何资源上添加ACL  
例如，假设您要添加ACL“允许主用户：Peter可以从IP 198.51.200.0的任何主题中产生”，您可以使用通配符资源“ *”来实现，例如，通过使用以下选项执行CLI：
...
IdempotentWrite
All  
这些操作对应的api见官网：http://kafka.apache.org/documentation/#operations_in_kafka  